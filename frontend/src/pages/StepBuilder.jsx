...
import {
  BookOpen, FileText, ClipboardList, Award,
  ChevronRight, Sparkles, CheckCircle, AlertTriangle,
  RefreshCw, Download, ArrowRight, Loader2, Search, Bot
} from 'lucide-react';
...
const STEPS = [
  ...
  },
  {
    ...
    title: 'Write Lessons',
    subtitle: 'Content Developer writes 300+ word lessons for each of the 5 modules.',
  ...
  },
  {
    ...
    subtitle: 'QA Lead audits everything and compiles the final course document.',
...
export default function StepBuilder() {
  ...
  const [stepStatuses, setStepStatuses] = useState({
    syllabus: 'idle', lessons: 'idle', quiz: 'idle', report: 'idle',
  });
  // Error messages per step
  const [stepErrors, setStepErrors] = useState({
    syllabus: null, lessons: null, quiz: null, report: null,
  });
  const [outputs, setOutputs] = useState({
    syllabus: '', lessons: '', quiz: '', final_report: '',
  });
  ...
  async function runStep(stepKey) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setStepErrors(prev => ({ ...prev, [stepKey]: err.detail || 'Error starting step' }));
        return;
      }
      // Sync with server
      await fetchAll();
      ...
      setStepStatuses(prev => ({ ...prev, [stepKey]: 'error' }));
      setStepErrors(prev => ({ ...prev, [stepKey]: 'Could not connect to backend. Is uvicorn running?' }));
    }
  }

  function handleDownload(key, filename) {
    const content = key === 'final_report' ? outputs.final_report : outputs[key];
    if (!content) return;
    ...
    <div style={{ position: 'relative', minHeight: '100vh', paddingTop: 80 }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, maxWidth: 960, margin: '0 auto', padding: '40px 24px 80px' }}>
        ...
        <div className="fade-in-up" style={{ marginBottom: 32 }}>
          <h1 style={{ fontWeight: 900, fontSize: '1.8rem', letterSpacing: '-0.02em', color: '#f1f5f9', marginBottom: 6 }}>
            Step-by-Step <span className="gradient-text">Builder</span>
          </h1>
          <p style={{ color: '#64748b', fontSize: '0.9rem' }}>
            Control each agent independently — review outputs at every stage before proceeding.
          </p>
        </div>

        {/* Topic input (shared) */}
        <div className="glass-card fade-in-up delay-100" style={{ padding: '20px 24px', marginBottom: 28 }}>
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'flex-end' }}>
            <div style={{ flex: 1, minWidth: 240 }}>
              <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 600, color: '#64748b', marginBottom: 6, letterSpacing: '0.05em', textTransform: 'uppercase' }}>
                Course Topic
              </label>
              <input
                id="step-topic-input"
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <button
                ...
                style={{
                  ...
                  color: '#64748b', display: 'flex', alignItems: 'center', gap: 6, fontSize: '0.82rem',
                }}
              >
                <RefreshCw size={14} />
                Refresh
          ...
          {knownTopics.length > 0 && (
            <div style={{ marginTop: 14 }}>
              <p style={{ fontSize: '0.7rem', fontWeight: 600, color: '#334155', letterSpacing: '0.05em', textTransform: 'uppercase', marginBottom: 8 }}>
                Previously Generated
              </p>
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                {knownTopics.map(t => {
                  const doneCount = ['syllabus','lessons','quiz','report'].filter(k => t[k]).length;
                  const isSelected = topic.trim().toLowerCase().replace(/ /g,'_') === t.slug;
                  return (
                    <button
                      key={t.slug}
                      onClick={async () => {
                        ...
                        setStepStatuses({ syllabus: 'idle', lessons: 'idle', quiz: 'idle', report: 'idle' });
                        setStepErrors({ syllabus: null, lessons: null, quiz: null, report: null });
                        setOutputs({ syllabus: '', lessons: '', quiz: '', final_report: '' });
                    ...
                    >
                      <span style={{
                        ...
                        background: doneCount === 4 ? '#34d399' : doneCount > 0 ? '#f59e0b' : '#475569',
                      }} />
                      {t.label}
            ...
            <div className="glass-card" style={{ overflow: 'hidden' }}>
              ...
              <div style={{
                ...
                <div style={{
                  ...
                  background: isDone ? 'linear-gradient(135deg,#10b981,#34d399)' : step.gradient,
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <h2 style={{ fontWeight: 700, fontSize: '1rem', color: '#f1f5f9' }}>{step.title}</h2>
                    <StepBadge status={stepStatus} />
                  </div>
                  <p style={{ fontSize: '0.78rem', color: '#475569', marginTop: 2 }}>{step.subtitle}</p>
                </div>
              </div>
              ...
              <div style={{ padding: 24 }}>
                ...
                {isError && (
                  <div style={{
                    ...
                    <div style={{ fontWeight: 700, marginBottom: stepError ? 6 : 0 }}>✗ Step failed</div>
                    {stepError && (
                      <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '0.76rem', opacity: 0.85, wordBreak: 'break-word' }}>
                        {stepError}
                      </div>
                    )}
                  </div>
                ...
                <div style={{ display: 'flex', gap: 10, marginBottom: 24, flexWrap: 'wrap' }}>
                  ...
                  >
                    {isRunning ? (
                      <><div className="spinner" style={{ width: 16, height: 16 }} /> Running...</>
                    ) : isDone ? (
                      <><RefreshCw size={15} /> Regenerate</>
                    ) : (
                      <><Sparkles size={15} /> {step.actionLabel} <ArrowRight size={13} /></>
                    )}
                  </button>

                  {isDone && (
                    <button
                      onClick={() => handleDownload(step.outputKey, step.outputLabel)}
                      style={{
                ...
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                    <span style={{ fontSize: '0.75rem', fontWeight: 600, color: '#334155', letterSpacing: '0.05em', textTransform: 'uppercase' }}>
                  ...
                  <div style={{
                    ...
                    borderRadius: 12, padding: '20px 22px', maxHeight: 440, overflowY: 'auto',
                    minHeight: 120,
                  }}>
                    {isRunning ? (
                      <RunningPlaceholder />
                      ...
                      <MarkdownViewer
                        content={step.outputKey === 'final_report' ? outputs.final_report : outputs[step.outputKey]}
...
function StepNav({ steps, activeStep, stepStatuses, onSelect }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      <p style={{ fontSize: '0.7rem', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: '#334155', marginBottom: 8, paddingLeft: 4 }}>
        Pipeline Steps
      </p>
      {steps.map((step, i) => {
        ...
        return (
          ...
          >
            ...
            <div style={{
              ...
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              fontSize: '0.8rem',
              boxShadow: (isDone || isRunning) ? `0 2px 10px ${isDone ? 'rgba(16,185,129,0.3)' : step.glow}` : 'none',
            }}>
              {isDone
            ...
            <div style={{ minWidth: 0 }}>
              ...
              {isRunning && (
                <div style={{ fontSize: '0.68rem', color: '#818cf8', marginTop: 2 }}>Running…</div>
              )}
              {isDone && (
                <div style={{ fontSize: '0.68rem', color: '#34d399', marginTop: 2 }}>Complete</div>
              )}
            </div>
          </button>
        );
...
function StepBadge({ status }) {
  ...
  return (
    <span className={cls} style={{
      ...
      {status === 'running' && <span style={{ display: 'inline-block', width: 5, height: 5, borderRadius: '50%', background: '#818cf8', marginRight: 4, verticalAlign: 'middle', animation: 'pulse-anim 1.2s ease-in-out infinite' }} />}
      {label}
    </span>
  );
}
...
function RunningPlaceholder() {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '40px 20px', gap: 14 }}>
      ...
      <p style={{ color: '#6366f1', fontWeight: 600, fontSize: '0.9rem' }}>Agent is working…</p>
      <p style={{ color: '#334155', fontSize: '0.78rem', textAlign: 'center', maxWidth: 300 }}>
        The AI agent is generating content. This may take a minute. Output will appear here automatically.
      </p>
    </div>
  );
}
